...
| `pin-model-010` | No preset pins. Any repo visible in the connected account/orgs is pinnable via an in-place row toggle (Claude Code session-pinning interaction); pinned repos sort first and form the widget and background-refresh set. The maintainer's twelve pins are personal config, not product defaults. | implemented (verified at PR #1 merge) |
