...
| `delivery-rail-012` | Merge → CI builds the APK → rolling GitHub release → cp-1 pulls within 30 minutes → update banner on the maintainer's phone over WireGuard → one-tap install (the SwarmPocket rail, generalized). The public rolling release doubles as the public sideload channel; puller/banner config stays swarm-side. First install is one manual sideload. The draft PR #3 rail now separates build/sign/publish, requires a continuity fingerprint plus environment-held signing secrets, retains immutable per-SHA payloads, and promotes the rolling manifest last; provisioning, merge, release activation, cp-1 deployment, and live end-to-end proof remain maintainer-gated. | implemented in draft; not active or live-proven |
