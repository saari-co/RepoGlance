...
| No disk or network I/O on the main thread | PR #12 review (`LiveSnapshotStore.load`) | StrictMode in debug builds (network kills, disk logs); verifier tiers fail on `StrictMode` log lines (Grill C). **First catches, 2026-09-18, fixed by `session-bootstrap-016`:** `SecureTokenStore` read the session file on the main thread during `RepoGlanceViewModel` construction (six `DiskReadViolation`s per cold start), and the debug picker's persisted-store candidate called `LiveSnapshotStore.load` in composition. The token file is now `lazy`, session reads and writes run on the ViewModel's `sessionDispatcher`, and the picker loads on `Dispatchers.IO` (`proof/session-bootstrap-20260918/PROOF.md`); a direct `MainActivity` cold start on 2026-09-19 logged no `DiskReadViolation`. The debug `ScenarioLaunchActivity` wrote `AppPrefs` and `RefreshProbe` state on the main thread (three `DiskReadViolation`s per verify launch); fixed 2026-09-19 by writing on `Dispatchers.IO` before starting the next screen, guarded by `ScenarioLaunchActivitySourceTest` (`proof/main-thread-debt-20260919/PROOF.md`). **Caught and fixed, 2026-09-19:** both widgets read `AppPrefs`, `LiveSnapshotStore`, `LiveRowsStore`, `RateLimitStore`, `RepoWidgetConfigStore` and `CatalogNamesStore` inside `provideContent` on every redraw (26 `DiskReadViolation`s after an `adb install -r`); Glance's `SessionWorker` runs `provideGlance` and its composition on `Dispatchers.Main`, so the reads now go through `readWidgetStores` (`Dispatchers.IO`) before `provideContent` and again from `redrawnWidgetData` on each `REDRAW_KEY` bump. Guarded by `WidgetStoreReadsTest` and `StackWidgetTest`. | 1 (runtime), debt recorded |
