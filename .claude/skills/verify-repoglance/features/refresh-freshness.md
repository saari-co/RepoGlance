...
- `fresh-failed` shows `Could not refresh GitHub right now` and `No current value` after a failed refresh, and ages the compact widget's persisted counts to `last good <age>`.
...
- Compact widget: the freshness label beside the repository name.
...
- **Widget consequence.** Run `bin/verify-repoglance launch MIXED picker` then `dump picker-offline`. The `PERSISTED` row reads `last good <age>` with counts preserved.
