...
- **Proof.** `capture navigator-issues` after the mode switch, `capture navigator-unknown` with `Data unavailable` showing and `capture navigator-last-good` with the `Cached · <age>` chip showing, `capture navigator-row-wide` with the GitHub pane open, `capture navigator-cover-detail` on the cover display when the maintainer has folded and unlocked it, and `capture navigator-home` after returning.
