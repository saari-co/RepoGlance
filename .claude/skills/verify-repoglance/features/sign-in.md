...
- `signin-code` shows the user code, `Copy code & open GitHub`, and `Cancel sign-in`, with an expiry countdown.
...
- **Maintainer step (gated).** The maintainer taps `Connect GitHub`, then `Copy code & open GitHub`, enters the code on GitHub, and returns. The agent may `dump code-screen` while the code is visible to record `repoglance:copy-code` and the `Enter this code on GitHub` text; it never reads, stores, or transmits the code value.
...
- **Proof.** Capture only screens that contain no code value and no unfiltered catalog: the `Connect GitHub` screen and the guarded filtered catalog from Find a repository.
...
- Never screenshot the code screen with the code legible; the dump text is enough and even that must not be quoted.
