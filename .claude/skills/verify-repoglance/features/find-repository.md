...
- `find-pin` pins a repository with its thumbtack; pinned repositories always lead the list (newest push first among pins) in either sort, and pins clear when the session ends.
...
- **Pin.** From the filtered view tap `Pin saari-co/RepoGlance` (content-desc), then clear the search and `dump pinned`: the first repository-shaped label is `saari-co/RepoGlance` and the row's control reads `Unpin saari-co/RepoGlance`. Re-filter before any capture. Tap `Unpin saari-co/RepoGlance` and `dump unpinned` to restore the order. Never capture the unfiltered list.
