...
| R4 | P3 | The guard test did not check each folder's parent theme, so a dark parent in `values-v34` would pass. | required_fix: `ColdStartIconGuardTest` now asserts the light or dark parent per folder. |
